        ...
        if (t && this.movementSpeed)
        {
            ...
            if (ts)
            {
                ...
                s.setDrawFunction(wade.drawFunctions.fadeOpacity_(1, 0, time, s.getDrawFunction()));
                 this.owner.objectFindThree = false;
            wade.app.score_toys = (wade.app.score_toys || 0) + 1;
           // _.scoreText.getSprite().setText(wade.app.score_cat);
            if (!wade.getSceneObjects('objectFindThree', true).length)
                {
                //   _.congratzMessage.setVisible(true);
                // _.timer.stop();
                }
                 this.totalObject =wade.app.score_toys;
